...
router.post("/approve/:id", protect, adminOnly, (req, res) => {
  ...
  const selectQuery = "SELECT email, first_name FROM users WHERE id = ?";
  db.query(selectQuery, [userId], (err, rows) => {
    if (err || rows.length === 0) return res.status(500).json({ message: "Cannot fetch user" });
    ...
    transporter.sendMail(mailOptions, (err2) => {
      if (err2) return res.status(500).json({ message: "Email sending failed", error: err2 });
      ...
      db.query(updateQuery, [activationToken, userId], (err3) => {
        if (err3) return res.status(500).json({ message: "Database update failed", error: err3 });
